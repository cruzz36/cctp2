...
2. **Fechar e reabrir topologia no CORE:**
   - Se alteraste o ficheiro `.imn`, **DEVES FECHAR E REABRIR a topologia no CORE**:
     ...
     2. File → Open → selecionar `config/topologiatp2.imn` atualizado
...
- Se alteraste o ficheiro `.imn`, **DEVES FECHAR E REABRIR a topologia no CORE**:
  ...
  2. File → Open → selecionar `config/topologiatp2.imn`
...
1. **Inicializar a topologia no CORE:**
   - File → Open → selecionar `config/topologiatp2.imn`
...
2. **Aplicar rotas automaticamente em todos os nós:**
   ...
   for NODE in NaveMae GroundControl Rover1 Rover2 Satelite; do
     ...
     sudo vcmd -c $SESSION/$NODE -- sh -c 'cd /tmp/nms && chmod +x scripts/verificar_rotas.sh && ./scripts/verificar_rotas.sh'
   done
   ```
...
- Zip (manual): no **host CORE** correr  
  `tar -czf nms_code.tar.gz src scripts data config requirements.txt --exclude='__pycache__' --exclude='*.pyc'`  
...
- Depois do `copy_to_core.py` (se a verificação por SSH falhar): abre o terminal de cada nó e confirma `ls /tmp/nms && test -f /tmp/nms/scripts/start_nms.py`. Se não existir, volta a extrair o tar no nó: `mkdir -p /tmp/nms && cd /tmp/nms && tar -xzf /tmp/nms_code.tar.gz`.
...
- Ordem para evitar timeout na Nave-Mãe: arrancar NaveMae e logo a seguir os rovers (Rover1, Rover2); só depois o GroundControl. Se a thread do MissionLink cair por timeout, relança `scripts/start_nms.py` depois de subires os rovers.
- comando clear do vcmd : `TERM=vt100 clear`
- **Nave-Mãe (n1)**  
  `cd /tmp/nms`  
  `python3 scripts/start_nms.py`

- **Rover1 (n3)**  
  `cd /tmp/nms`  
  `python3 scripts/start_rover.py 10.0.1.10 r1`

- **Rover2 (n4)**  
  `cd /tmp/nms`  
  `python3 scripts/start_rover.py 10.0.1.10 r2`

- **Ground Control (n2)**  
  `cd /tmp/nms`  
  `python3 scripts/start_ground_control.py`

### 5) Testar
- Em qualquer nó: `cd /tmp/nms && python3 scripts/test_core_automated.py auto`
- Em qualquer nó: `cd /tmp/nms && chmod +x scripts/test_core_integration.sh && ./scripts/test_core_integration.sh`
- Quando usar cada teste:
  - Antes de copiar para os nós (no host): `python3 scripts/test_imports.py`
  - Após extrair em cada nó, antes de arrancar serviços: `python3 scripts/test_core_automated.py auto`
  - `test_core_integration.sh` é só um wrapper para o teste acima (em `/tmp/nms/scripts`)
- Interpretação rápida:
  - Se `test_core_automated.py` falhar por porta ocupada: pare serviços (`pkill -f scripts/start_nms.py`, `fuser -k 8080/udp 8081/tcp 8082/tcp`) e repita.
  - Se falhar import/ficheiro: falta algo em `/tmp/nms` ou deps; recopie (passo 2) e reinstale deps (passo 3).
  - Para validar comunicação real, execute os testes e verifique os logs dos serviços a correr.
...
- "Module not found": confirma `/tmp/nms/src` e `__init__.py` nos subdiretórios.
- Porta ocupada: `pkill -f scripts/start_nms.py` ou `pkill -f scripts/start_rover.py`.
...
- Em cada nó (NaveMae, GroundControl, Rover1, Rover2):
  3. Parar processos antigos: `pkill -f scripts/start_nms.py || true && pkill -f scripts/start_rover.py || true && pkill -f scripts/start_ground_control.py || true`
...
- Automático pós-pull (sem internet nos nós) — tudo no host CORE:
  ...
  for NODE in NaveMae GroundControl Rover1 Rover2; do
    sudo sh -c "vcmd -c $SESSION/$NODE -- sh -c 'pkill -f scripts/start_nms.py || true; pkill -f scripts/start_rover.py || true; pkill -f scripts/start_ground_control.py || true; rm -rf /tmp/nms; mkdir -p /tmp/nms'"
...
sudo sh -c "cat /home/core/pkgs.tgz | vcmd -c /tmp/pycore.38475/NaveMae -- sh -c 'tar -xzf - -C /tmp && pip3 install --no-index --find-links /tmp/pkgs flask==1.1.1 itsdangerous==1.1.0 jinja2==2.10.1 markupsafe==1.1.1 werkzeug==0.16.1 click==7.0 psutil==5.9.0 requests==2.22.0'"

     sudo sh -c "cat /home/core/pkgs.tgz | vcmd -c /tmp/pycore.38475/GroundControl -- sh -c 'tar -xzf - -C /tmp && pip3 install --no-index --find-links /tmp/pkgs flask==1.1.1 itsdangerous==1.1.0 jinja2==2.10.1 markupsafe==1.1.1 werkzeug==0.16.1 click==7.0 psutil==5.9.0 requests==2.22.0'"

     sudo sh -c "cat /home/core/pkgs.tgz | vcmd -c /tmp/pycore.38475/Rover1 -- sh -c 'tar -xzf - -C /tmp && pip3 install --no-index --find-links /tmp/pkgs flask==1.1.1 itsdangerous==1.1.0 jinja2==2.10.1 markupsafe==1.1.1 werkzeug==0.16.1 click==7.0 psutil==5.9.0 requests==2.22.0'"

     sudo sh -c "cat /home/core/pkgs.tgz | vcmd -c /tmp/pycore.38475/Rover2 -- sh -c 'tar -xzf - -C /tmp && pip3 install --no-index --find-links /tmp/pkgs flask==1.1.1 itsdangerous==1.1.0 jinja2==2.10.1 markupsafe==1.1.1 werkzeug==0.16.1 click==7.0 psutil==5.9.0 requests==2.22.0'"
