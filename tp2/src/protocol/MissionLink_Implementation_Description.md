...
- **Porquê enviar ID do rover no handshake?**: Embora `recvfrom()` retorne o endereço IP/porta do remetente, o servidor precisa do **ID do rover** (identidade lógica fixa) para mapear `ID do rover → IP/porta` no dicionário `agents`. Isto permite:
  - Rastrear rovers mesmo quando IP/porta mudam
  - Distinguir múltiplos rovers (útil em cenários NAT)
  - Enviar missões ao rover correto usando `agents[rover_id]`
