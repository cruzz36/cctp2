...
def validateMission(mission_data):
    ...
    Campos opcionais:
    - "instructions": string
    - "update_frequency_seconds": integer (frequência em segundos para reportar progresso, padrão: 27)
...
class Server: 
    ...
    def registerAgent(self,idAgent,ip):
        ...
        Carrega automaticamente missões do diretório data para o rover registado.
        ...
        if self.agents.get(idAgent) == None:
            ...
            # Carregar missões do diretório data para este rover
    ...
    def _loadMissionsForRover(self, rover_id):
        """
        Carrega automaticamente missões do diretório data para um rover específico.
        ...
        """
        # Tentar múltiplos caminhos possíveis para data
        possible_paths = [
            "data",  # Diretório atual
            "/tmp/nms/data",  # Caminho padrão no CORE
            os.path.join(os.path.dirname(__file__), "..", "..", "data"),  # Relativo ao módulo (src/server -> data)
        ]
        ...
        import glob
        mission_files = glob.glob(os.path.join(data_dir, "mission*.json"))
    ...
    def handleMissionRequest(self, idAgent, ip):
        ...
        # Se não encontrou missão específica, verificar se há mais missões no diretório data para este rover
        if mission_to_send is None:
            # Se não há missões pendentes específicas, verificar se há mais missões no diretório data para este rover
            self._loadMissionsForRover(idAgent)
    ...
    def handleMissionProgress(self, idAgent, idMission, progress_json, ip):
        ...
        Mostra no terminal os dados recebidos (progresso e bateria).
        """
        try:
            ...
            # Extrair informações para mostrar no terminal
            if isinstance(progress_data, dict):
                status = progress_data.get("status", "unknown")
                progress_percent = progress_data.get("progress_percent", 0)
                battery = progress_data.get("battery", 0.0)
                position = progress_data.get("current_position", {})
                elapsed_time = progress_data.get("elapsed_time_seconds", 0)
                
                # Mostrar dados recebidos no terminal
                if status == "completed":
                    print(f"[Nave-Mãe] Missão {idMission} CONCLUÍDA pelo rover {idAgent}")
                    print(f"         Progresso: {progress_percent}% | Bateria: {battery:.1f}% | Tempo decorrido: {elapsed_time}s")
                    if idMission in self.tasks:
                        del self.tasks[idMission]
                else:
                    pos_str = f"({position.get('x', 0):.1f}, {position.get('y', 0):.1f}, {position.get('z', 0):.1f})"
                    print(f"[Nave-Mãe] Update de Missão {idMission} recebido do rover {idAgent}")
                    print(f"         Progresso: {progress_percent}% | Bateria: {battery:.1f}% | Posição: {pos_str} | Tempo: {elapsed_time}s")
            
            # Enviar confirmação
            self.missionLink.send(ip, self.missionLink.port, None, idAgent, idMission, "progress_received")
            
        except json.JSONDecodeError as e:
            print(f"[ERRO] Nave-Mãe: Erro ao fazer parse do progresso da missão {idMission} do rover {idAgent}: {e}")
            self.missionLink.send(ip, self.missionLink.port, None, idAgent, idMission, "parse_error")
        except Exception as e:
            print(f"[ERRO] Nave-Mãe: Erro ao processar progresso da missão {idMission} do rover {idAgent}: {e}")
