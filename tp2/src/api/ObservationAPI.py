...
class ObservationAPI:
    ...
    def __init__(self, nms_server, host='0.0.0.0', port=8082):
        ...
        Args:
            nms_server (Server): Instância do servidor NMS para aceder aos dados
    ...
    def start(self):
        ...
        def run_api():
            ...
            try:
                print(f"[API] A iniciar API de Observação (porta {self.port})")
        ...
        if self._api_thread.is_alive():
            print(f"[OK] API de Observação iniciada (porta {self.port})")
        else:
