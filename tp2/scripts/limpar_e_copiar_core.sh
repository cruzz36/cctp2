...
if [ ! -f "$PROJECT_DIR/scripts/copy_to_core.py" ]; then
    echo "ERRO: scripts/copy_to_core.py não encontrado no diretório atual!"
    echo "Diretório atual: $PROJECT_DIR"
    echo "Certifica-te de que estás no diretório do projeto (deve conter scripts/copy_to_core.py)"
    exit 1
fi
...
if [ -f "scripts/copy_to_core.py" ]; then
    python3 scripts/copy_to_core.py || {
        echo "ERRO: Falha ao gerar código com scripts/copy_to_core.py"
        exit 1
    }
else
    echo "ERRO: scripts/copy_to_core.py não encontrado!"
    exit 1
fi
...
for NODE in NaveMae GroundControl Rover1 Rover2; do
    ...
    if sudo vcmd -c $SESSION/$NODE -- sh -c 'test -f /tmp/nms/scripts/start_nms.py || test -f /tmp/nms/scripts/start_rover.py'; then
...
echo "1. Aplicar rotas de rede (ver docs/GUIA_TESTE_CORE.md PASSO 3)"
echo "2. Instalar dependências se necessário (pip3 install -r requirements.txt)"
echo "3. Arrancar serviços nesta ordem:"
echo "   - NaveMae: cd /tmp/nms && python3 scripts/start_nms.py"
echo "   - Rover1: cd /tmp/nms && python3 scripts/start_rover.py 10.0.1.10 r1"
echo "   - Rover2: cd /tmp/nms && python3 scripts/start_rover.py 10.0.1.10 r2"
echo "   - GroundControl: cd /tmp/nms && python3 scripts/start_ground_control.py"
echo ""
