...
def verify_node(node_name, node_info, core_session=None):
    ...
    if core_session:
        try:
            cmd = f"vcmd -c {core_session}/{node_name} -- sh -c 'test -d /tmp/nms && test -f /tmp/nms/scripts/start_nms.py && echo OK || echo FAIL'"
            result = subprocess.run(cmd, shell=True, capture_output=True, text=True, timeout=5)
    ...
    try:
        cmd = f"ssh -o StrictHostKeyChecking=no -o ConnectTimeout=5 root@{node_ip} 'test -d /tmp/nms && test -f /tmp/nms/scripts/start_nms.py && echo OK || echo FAIL'"
        result = subprocess.run(cmd, shell=True, capture_output=True, text=True, timeout=5)
...
def main():
    ...
    if success_count == len(NODES):
        ...
        print("   chmod +x scripts/apply_routes.sh")
