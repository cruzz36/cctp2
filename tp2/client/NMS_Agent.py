...
class NMS_Agent:
    ...
    def executeMission(self, mission_data, server_ip):
        ...
        Reporta progresso da missão de acordo com update_frequency_seconds definido na missão.
        ...
        # Extrair frequência de atualização de progresso (padrão: 27 segundos)
        update_frequency_seconds = mission_data.get("update_frequency_seconds", 27)
        
        print(f"[INFO] Executando missão {mission_id} ({task}, {duration_minutes}min, progress updates a cada {update_frequency_seconds}s)")
        ...
        last_progress_report_time = start_time  # Controlar quando reportar progresso via MissionLink
        ...
        # Função auxiliar para reportar progresso via MissionLink
        def report_mission_progress(current_progress_percent):
            """Reporta progresso da missão via MissionLink (protocolo UDP) com fragmentação se necessário."""
            try:
                progress_data = {
                ...
                # Enviar via MissionLink (UDP) - tipo "P" (Progress)
                # O MissionLink fragmenta automaticamente se a mensagem for grande
                success = self.missionLink.send(server_ip, self.missionLink.port, self.missionLink.reportProgress, self.id, mission_id, progress_json)
                if success:
                    print(f"[INFO] Update de Missao {mission_id} (Progresso: {current_progress_percent}%, Bateria: {self.battery:.1f}%) enviada para a NaveMae")
                else:
                    print(f"[ERRO] Falha ao enviar update de missão {mission_id} para a NaveMae")
            except Exception as e:
                print(f"[ERRO] Erro ao reportar progresso da missão {mission_id}: {e}")
        ...
        while True:
            ...
            # Reportar progresso via MissionLink de acordo com update_frequency_seconds
            time_since_last_report = time.time() - last_progress_report_time
            if time_since_last_report >= update_frequency_seconds:
                report_mission_progress(progress_percent)
